...
% Updated by: Jennifer Hsu, 04/26/2011
...
crystals_from_crystallizer = (0.9)*Feed_to_crystallizer(3); % 90% of moles of P crystallize
weight_of_crystals = crystals_from_crystallizer*Materials_Properties(3,1)/1000; % Kilograms of crystals, P
liquid_coating_crystals = (0.1)*weight_of_crystals; % kg of liquid coating the crystals in the crystallizer
liquid_after_removal_of_crystals(1:9) = Feed_to_crystallizer(1:9) - [0 0 crystals_from_crystallizer 0 0 0 0 0 0];
% The next line computes the mole fractions of all components in the liquid
% at the end of crystallization, e.g. (mole fraction of A) = (moles of
...
% The next line computes the weight ratios of all components in the liquid
% with respect to chemical, A, e.g.(weight ratio of B) = (weight of
% B)/(weight of A).
...
% The next line computes the sum of the weight ratios, i.e. sum(weight
% ratio of A, weight ratio of B, etc.)
sum_of_weight_ratios = sum(weight_ratios_in_liquid_after_removal_of_crystals(1:9));
% The next line computes the moles of A in the liquid coating of the
% crystals as follows: The sum of the weights of all components in the
% liquid coating the crystals is equal to the "liquid_coating_crystals",
% computed in line 28, above. Therefore, (weight,A) =
% (liquid_coating_crystals)/(sum(weight ratios*ratios of molecular weights),
% and (moles,A) = (weight,A)*1000/(molecular weight, A); moles of A in the
% liquid coating the crystals.
moles_in_the_liquid_coating_the_crystals(1) = 1000*(liquid_coating_crystals)/(sum_of_weight_ratios*Materials_Properties(1,1));
% The next line yields the moles of B,P,Q,W,Z,C,S1,S2 in the liquid coating
% the crystals
...
% Define a new vector,
% "Crystal_phase_Coating_Liquid_from_crystallizer(1:9)", that describes the
% moles of A,B,P,Q,W,Z,C,S1,S2 in the liquid coating the crystals. 
% "Crystal_phase_Coating_Liquid_from_crystallizer(1:9)" describes the
% composition of the liquid that leaves the crystallizer with the crystals.
...
Crystal_phase_from_crystallizer(3) = crystals_from_crystallizer;
