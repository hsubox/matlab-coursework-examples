...
global Materials_Costs Utilities_Costs Vessel_Rental_Costs Labor_Costs Materials_Credits Materials_Prices
...
Distillation_overhead_product(9) = (0.01/0.99)*(distillation_feed(1) + distillation_feed(2)); % moles of S2
Distillation_overhead_product(3:8) = 0; % moles of P,Q,W,Z,C,S1
Distillation_overhead_product(10) = 298; % temperature in degrees K
...
% Description of Distillation Bottoms stream
Distillation_bottoms_product(1) = distillation_feed(1) - Distillation_overhead_product(1); % moles of A
Distillation_bottoms_product(2) = distillation_feed(2) - Distillation_overhead_product(2); % moles of B
Distillation_bottoms_product(9) = distillation_feed(9) - Distillation_overhead_product(9); % moles of S2
Distillation_bottoms_product(3:8) = distillation_feed(3:8); % moles of P,Q,W,Z,C,S1
Distillation_bottoms_product(10) = 458; % temperature in degrees K
...
heating_duty_for_sensible_heating = sum((distillation_feed(1:9)'./(1e3*Materials_Properties((1:9),1))).*Materials_Properties((1:9),3)*(458-Extractor_Phase_2_exit_stream(10))); % in KJ. Heat used to heat distillation feed from its temperature to 458 degrees Kelvin
heating_duty_for_vaporization = Distillation_overhead_product(1:9)*Materials_Properties((1:9),4); % in KJ. Heat used to vaporize the moles of A, B, and S2, which go to the distillate stream
total_heating_duty = heating_duty_for_sensible_heating + heating_duty_for_vaporization;

% Cooling duty in the condenser
% cooling_duty_for_sensible_cooling = xxxxxxxxx; % in KJ. Heat removed during cooling of the distillate from 458 to 298 degrees Kelvin
total_cooling_duty = heating_duty_for_vaporization; % in KJ. The cooling duty is equal to the amount of heating used for vaporization
% 
% Compute the Economics of the Distillation per batch
% 
distillation_period = 18/3600*(distillation_feed(1) + distillation_feed(2));                            % in hours
distillation_rental_cost = (distillation_period + DistillationTurn)*Distillation_rental_cost_per_hour;  % in $
...
% Labor Cost
distillation_labor_cost = (distillation_period + DistillationTurn)*Labor_unit_cost; % in $
% 
...
Vessel_Occupancy(3) = distillation_period + DistillationTurn; % Total occupancy time of the distillation system, per batch
